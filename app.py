...
f_str = st.text_input("Entrez votre fonction non linéaire f(x) (en termes de x):", "x**2 - 2")

# Entrée des bornes de l'intervalle
x0 = st.number_input("Entrez la première borne de l'intervalle (x0):", value=1.0)
x1 = st.number_input("Entrez la seconde borne de l'intervalle (x1):", value=2.0)

# Paramètres de la méthode de la sécante
tol = 1e-10  # Tolérance pour l'erreur de la solution
max_it = 1000  # Nombre maximum d'itérations

# Fonction d'évaluation de f(x)
def eval_func(f_str, x):
    return eval(f_str, {"__builtins__": {}}, {"x": x, "math": math})

# Méthode de la sécante
if st.button("Résoudre l'équation"):
    it = 0
    r1 = eval_func(f_str, x0)
    r2 = eval_func(f_str, x1)
